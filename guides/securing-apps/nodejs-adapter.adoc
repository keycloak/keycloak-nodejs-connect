...
In that case, a {project_name} deployment is necessary so you can access the {project_name} Admin Console.

<@profile.ifCommunity>
To deploy a {project_name} Admin Console, see 
...
Once the Admin Console is deployed, we are ready to obtain the `keycloak.json` file by visiting the {project_name} Admin Console -> clients (left sidebar) -> choose your client -> Installation -> Format Option -> Keycloak OIDC JSON -> Download.
...
ifeval::[{project_community}==true]
For a complete example of using the Node.js adapter, see {quickstartRepo_link}/tree/main/nodejs/resource-server[Keycloak quickstarts for Node.js].
endif::[]

ifeval::[{project_product}==true]
For a complete example of using the Node.js adapter, see the https://github.com/redhat-developer/rhbk-quickstarts[{project_name} quickstarts] repository.
endif::[]
